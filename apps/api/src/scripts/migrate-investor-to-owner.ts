...
async function migrateInvestorToOwner() {
  ...
  try {
    ...
    console.log(`📊 Найдено пользователей с ролью 'investor': ${investors.length}`);
    ...
    if (remainingInvestors.length === 0) {
      console.log('✅ Миграция завершена успешно! Все пользователи с ролью "investor" обновлены на "owner"\n');
    } else {
      console.log(`⚠️  Внимание: осталось ${remainingInvestors.length} пользователей с ролью "investor"\n`);
    }
